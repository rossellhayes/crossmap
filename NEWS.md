...
* `xmap_raw()` and `future_xmap_raw()` are now defunct to reflect the planned removal of `pmap_raw()` from the `purrr` package and `future_pmap_raw()` from the `furrr` package (#15).
